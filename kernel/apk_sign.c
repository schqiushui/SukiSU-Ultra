...
static struct apk_sign_key {
	...
	{EXPECTED_SIZE_RIFSXD, EXPECTED_HASH_RIFSXD}, // rifsxd/KernelSU-Next
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
	{EXPECTED_SIZE_5EC1CFF, EXPECTED_HASH_5EC1CFF}, // MKSU
	{EXPECTED_SIZE_YUKONGA, EXPECTED_HASH_YUKONGA}, // YuKongA/KernelSU
...
static int verify_signature_block(struct file *fp, u32 *size4, loff_t *pos, u32 *offset, int *matched_index)
{
	...
	*offset += 0x4 * 2;
	...
	for (i = 0; i < ARRAY_SIZE(apk_sign_keys); i++) {
		...
		if (i == 1) {
			if (ksu_get_dynamic_sign_config(&sign_key.size, &sign_key.sha256)) {
				// sign_key now has valid dynamic values
			}
		}
